...
class RamanSpectrum:
    def __init__(self, img_path):
        self.img_path = f'data/img/raman_img/{img_path}'
    ...
    def get_spectrum(self, file_name):
        ...
        plt.savefig(f"data/img/spectrum_graph/{file_name}.png")

    def save_raw_spectrum(self, file_name):
        df = pd.DataFrame(
            {
    ...
    def open_csv(self, file_name):
        X = []
        Y = []
        with open(f"data/csv/{file_name}", "r") as f:
    ...
    def outFigCSV(self, input_path, output_name):  # baseline estimation and smoothing
        X, Y = self.open_csv(input_path)
        ...
        df.to_csv(f"data/csv/processed_{output_name}.csv")
        ...
        plt.legend()
        plt.axis("tight")
        plt.savefig(f"data/img/spectrum_graph/processded_{output_name}.png")

    def substruct_two_spectrum(self, file_name1, file_name2, file_name3):  # substruct two spectrum
        X1, Y1 = self.open_csv(file_name1)
        X2, Y2 = self.open_csv(file_name2)
        Y1_np = np.array(Y1)
        Y2_np = np.array(Y2)
        Y_substruct = Y1_np - Y2_np
        df = pd.DataFrame({'Wavenumber': X1, 'Substructed_Intensity': Y_substruct})
        df.to_csv(f'data/csv/{file_name3}.csv')
        # プロットする
        plt.xlabel('Wavenumber (cm-1)')
        plt.ylabel('Substructed Intensity')
        plt.plot(X1, Y_substruct)
        plt.savefig(f'data/img/spectrum_graph/{file_name3}.png')